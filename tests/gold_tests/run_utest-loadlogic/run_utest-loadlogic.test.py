...
t1 = Test.AddBuildRun('USE_CACHE_KEY=the.same run_utest:: -j2 --verbose=loading LOG_ROOT_DIR=#logs_1')
t1.ReturnCode = 0
t1Log = t1.Disk.File('logs_1/all.log')

t2 = Test.AddBuildRun('USE_CACHE_KEY=the.same run_utest::name::main:: -j2 --verbose=loading LOG_ROOT_DIR=#logs_2')
t2.ReturnCode = 0
t2Log = t2.Disk.File('logs_2/all.log')

TEST_LINES = [r'Verbose: \[loading\] Loading everything as (there is no cache|the given targets are unknown)']
...
t1Log.Exists = True
t1Log.Content = Testers.FileContentCallback(lambda data: checkPresence(data, TEST_LINES), description='Checking logs_1/all.log')
