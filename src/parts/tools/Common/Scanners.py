            ...
            for enity in os.listdir(path):
                ...
                if os.path.isdir(fullpath):
                    ...
                    if result:
                        ...
                        api.output.verbose_msgf(["generic.toolscanner", "toolscanner"],
                                                "Directory pattern matched: {0} version: {1}", enity, version_path)
                        ...
                        for subpath in self.subpaths:
                            ...
                            if version:
                                api.output.verbose_msgf(["generic.toolscanner", "toolscanner"],
                                                        "Adding: {0} version: {1}", fullpath, version)
    ...
    def get_tool_version(self, path) -> str:
        ...
        for line in pipe.stdout:
            ...
            if match:
                ...
                api.output.verbose_msgf(["generic.toolscanner", "toolscanner"],
                                        "Tool pattern matched: {0} version: {1}", path, version)
                return version
        return None
