...
def RedHatToolsetPaths(regex: str = r"(gcc\-|dev)toolset\-\d+") -> List[str]:
    '''Return the bin directories of Red Hat-style toolsets installed under
    /opt/rh. Matches both EL6-7 ``devtoolset-<n>`` and EL8+ ``gcc-toolset-<n>``
    by default. Returns an empty list when /opt/rh does not exist, so it is
    safe to splice into opt_dirs on any platform.'''
    RH_OPT_PATH: Final[str] = "/opt/rh"
    RH_OPT_ROOT_SUB_PATH: Final[str] = "root/usr/bin"

    pattern = re.compile(regex)

    if os.path.exists(RH_OPT_PATH):
        return [os.path.join(RH_OPT_PATH, d, RH_OPT_ROOT_SUB_PATH)
                for d in os.listdir(RH_OPT_PATH) if pattern.fullmatch(d)]
    return []
...
class GnuInfo(ToolInfo):
    ...
    def __init__(self, install_scanner, opt_dirs, script, subst_vars, shell_vars, test_file,
                 ...
                 test_file_prefix_pattern: Optional[str] = None,
                 test_file_suffix_pattern: Optional[str] = DEFAULT_TEST_FILE_SUFFIX_PATTERN,
    ...
    def scan_query(self, install_root: Optional[str], opt_scan: bool = True):
        ...
        def set_found_tuple(path, item):
            ...
            if version := self.get_default_ver(pathwtool):
                api.output.trace_msgf(['gnu_info', "tool_info"],
                                      "query scan found[{0}] = ({1}, {2})", version, path, pathwtool)
                found[version] = (path, pathwtool)

        if not self.found:
            test_file_pattern = re.compile(
                self.test_file_prefix_pattern + self.test_file.replace('+', r'\+') + self.test_file_suffix_pattern,
                re.IGNORECASE)
            if opt_scan:
                ...
                for d in self.opt_paths:
                    ...
                    if os.path.isdir(d):
                        api.output.verbose_msgf(['gnu_info', "tool_info"], "query scan opt_root={0}", d)
                        for item in os.listdir(d):
                            # look for a direct file match (dir/tool[-ver])
                            if self.test_file == item or test_file_pattern.fullmatch(item):
                                set_found_tuple(d, item)
                            # look for a versioned subdir (dir/tool-ver/bin/tool)
                            if opt_dir_pattern.match(item):
                                fullpath = os.path.join(d, item, 'bin')
                                if not os.path.exists(fullpath):
                                    continue
                                api.output.verbose_msgf(['gnu_info', "tool_info"], "query scan opt_root={0}", fullpath)
            ...
            if install_root:
                api.output.verbose_msgf(['gnu_info', "tool_info"], "query scan install_root={0}", install_root)
                ...
                # tool and tool-suffix, e.g. `gcc`, `gcc-14`, `clang-15.2`.
                # overrides anything discovered in opt_scan
                for item in os.listdir(install_root):
                    if self.test_file == item or test_file_pattern.fullmatch(item):
                        set_found_tuple(install_root, item)

                # see if we can find a default (unversioned) tool in the install root
                # overrides any previous finds of the same version
        ...
        if not self.found:
            ...
            return None
        api.output.verbose_msgf(['gnu_info', "tool_info"], "Found:\n{0}", pprint.pformat(self.found))
        return self.found
