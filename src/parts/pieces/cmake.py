...
def CMake(env:SConsEnvironment, prefix:str="$PACKAGE_ROOT", cmake_dir:Union[str,Dir]=None, auto_scanner={}, ignore:List[str]=[], top_level:bool=True, hide_c_flags:bool=False, **kw):
    ...
    ret = env.CCommand(
        ...
        out + src_files,
        [
            f"cd ${{SOURCE.dir}} ; $CMAKE_WRAPPER $CMAKE --build . --config Release --target install -- $CMAKE_DESTDIR_FLAG $_CMAKE_MAKE_ARGS"
        ],
        source_scanner=scanners.NullScanner,
...
api.register.add_variable('CMAKE_WRAPPER', '', 'Defines a tool to wrap around CMake (e.g. emcmake)')
