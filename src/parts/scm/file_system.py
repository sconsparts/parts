...
class file_system(base):
    ...
    Mapped as ScmFileSystem and VcsPreBuilds
    """
    __slots = []
    ...
    def UpdateAction(self, out_dir):
        ...
        return self._env.Action(cmdlst, "ScmFileSystem: Updating Files from %s to %s" % (self.FullPath, out_dir))
    ...
    def CheckOutAction(self, out_dir):
        ...
        return self._env.Action(cmdlst, "ScmFileSystem: Copying Files from %s to %s" % (self.FullPath, out_dir))
    ...
    def UpdateEnv(self):
        ...
        self._env['SCM'] = common.namespace(
            TYPE='file_system',
            CHECKOUT_DIR='$SCM_FILESYSTEM_DIR',
        )
